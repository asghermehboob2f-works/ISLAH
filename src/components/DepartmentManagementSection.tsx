      ...
      {modalMode && (
        <>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 overflow-y-auto font-sans pointer-events-none">
            ...
            {/* Modal Header */}
            <div className="bg-[--bg-subtle] text-[--text-primary] px-6 py-4 flex items-center justify-between border-b border-[--border] shrink-0">
              <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => setModalMode(null)}
                className="text-[--text-secondary] hover:text-[--text-primary] p-1.5 rounded-lg hover:bg-[--bg-surface] transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmitForm} className="p-6 overflow-y-auto space-y-6 flex-1 text-xs text-[--text-primary] bg-[--bg-surface]">
        ...
        <>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 overflow-y-auto font-sans pointer-events-none">
            ...
            <div className="flex items-center justify-between border-b border-[--border] pb-3">
              <div className="flex items-center gap-2">
                <span className="font-mono font-bold bg-[--bg-subtle] border border-[--border] text-[--text-primary] text-xs px-2.5 py-1 rounded">
                  {selectedDeptForView.code}
                </span>
                <h2 className="text-lg font-bold text-[--text-primary]">{selectedDeptForView.name}</h2>
              </div>
              <button
                onClick={() => setSelectedDeptForView(null)}
                className="text-[--text-secondary] hover:text-[--text-primary] p-1 rounded-lg hover:bg-[--bg-subtle] transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex justify-end pt-3 border-t border-[--border]">
              <button
                onClick={() => setSelectedDeptForView(null)}
                className="px-4 py-2 bg-[--text-primary] text-[--bg-base] font-bold text-xs rounded-md hover:opacity-90 transition-opacity"
              >
                Close Window
              </button>
            </div>
