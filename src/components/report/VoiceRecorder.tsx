...
export function VoiceRecorder({ onAudioSaved, initialAudioUrl = '' }: VoiceRecorderProps) {
  ...
  return (
    <div className="bg-[--bg-subtle] border border-[--border] rounded-xl p-4 space-y-3 font-sans">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-semibold text-[--text-secondary] uppercase">Accessibility Option</span>
      </div>
      ...
      {permissionError && (
        <div className="bg-amber-500/10 border border-amber-500/20 text-amber-700 dark:text-amber-400 p-2.5 rounded-lg text-xs flex items-center gap-2">
      ...
      {isRecording && (
        <div className="bg-red-500/10 border border-red-500/20 p-3 rounded-xl flex items-center justify-between">
          ...
          <button
            ...
            className="bg-red-600 hover:bg-red-700 text-white font-semibold text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-xs cursor-pointer"
          >
      ...
      {!isRecording && audioUrl && (
        <div className="bg-[--bg-surface] border border-[--border] p-3 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-xs font-bold text-[--text-primary] flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Voice Note Ready
              </div>
          ...
          <button
            ...
            className="text-xs font-semibold text-red-600 hover:text-red-700 hover:bg-red-500/10 px-2.5 py-1.5 rounded-lg border border-red-500/20 flex items-center gap-1 shrink-0 self-start sm:self-auto cursor-pointer"
          >
      ...
      {!isRecording && !audioUrl && (
        <button
          ...
          className="w-full bg-[--bg-surface] hover:bg-[--bg-subtle] border border-[--border] font-semibold text-xs text-[--text-primary] py-2.5 rounded-xl flex items-center justify-center gap-2 transition-all shadow-xs cursor-pointer"
        >
          <Mic className="w-4 h-4 text-blue-600 dark:text-blue-400" />
